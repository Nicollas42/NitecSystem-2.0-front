        ...
        <div class="linha_add">
            ...
            <div class="grupo_input" style="flex: 2; position: relative;">
                ...
                <input 
                    ...
                    v-model="termo_busca" 
                    class="input_padrao" 
                    placeholder="Digite para buscar (Ex: Farinha)..."
...
// Props para receber dados do pai
const props = defineProps(['lista']); 
const emit = defineEmits(['atualizar-total']);

// Estado
const lista_insumos_db = ref([]); // Lista completa vinda do banco
const termo_busca = ref('');
const mostrar_lista = ref(false);
const unidade_selecionada = ref(''); // Para mostrar no label (KG, LT, UN)

// Lista local da ficha técnica
const ingredientes_da_receita = ref([]); 

// Objeto Temporário para Adição
...
const carregar_insumos = async () => {
    ...
    try {
        const res = await axios.get('http://127.0.0.1:8000/api/produtos', {
            ...
            params: { loja_id: lojaId } // Filtra estoque da loja correta
        });
        
        // Filtra apenas o que é INSUMO (Matéria-prima)
...
const lista_filtrada = computed(() => {
    const termo = termo_busca.value ? termo_busca.value.toLowerCase() : '';
    
    return lista_insumos_db.value.filter(i => 
        i.nome.toLowerCase().includes(termo) || 
        String(i.id).includes(termo)
...
const fechar_lista_delay = () => {
    // Pequeno delay para permitir o clique antes de fechar
    setTimeout(() => { mostrar_lista.value = false; }, 200);
};
...
// Se vier lista do pai (edição futura), carrega aqui
...
const formatar_qtd = (v) => parseFloat(v || 0).toFixed(3).replace('.', ','); // Fix NaN
